    ...
    public class ProjectTasksController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<OCC.Shared.DTOs.DashboardUpdateDto>>> GetRecentUpdates()
        {
            try
            {
                ...
                var projectIds = topTasks.Select(t => t.ProjectId).Distinct().ToList();
                ...
                var dtos = topTasks.Select(t => 
                {
                    ...
                    if (string.IsNullOrEmpty(pName) && t.ProjectId != Guid.Empty)
                    {
                        projectMap.TryGetValue(t.ProjectId, out pName);
                    }
