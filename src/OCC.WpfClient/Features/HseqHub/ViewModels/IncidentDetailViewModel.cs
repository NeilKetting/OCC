using CommunityToolkit.Mvvm.ComponentModel;
using CommunityToolkit.Mvvm.Input;
using OCC.WpfClient.Services.Interfaces;
using OCC.WpfClient.Infrastructure;
using OCC.Shared.Enums;
using OCC.Shared.Models;
using OCC.Shared.DTOs;
using System;
using System.Collections.ObjectModel;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;
using System.IO;
using OCC.WpfClient.Services.Infrastructure;

namespace OCC.WpfClient.Features.HseqHub.ViewModels
{
    public partial class IncidentDetailViewModel : OverlayViewModel
    {
        private readonly IHealthSafetyService _hseqService;
        private readonly IAuthService _authService;
        private readonly ConnectionSettings _settings;

        [ObservableProperty]
        private Incident _incident = new() { Date = DateTime.Now };

        [ObservableProperty]
        private ObservableCollection<IncidentPhotoDto> _photos = new();

        [ObservableProperty]
        private ObservableCollection<IncidentDocumentDto> _documents = new();

        public IncidentType[] IncidentTypes => Enum.GetValues<IncidentType>();
        public IncidentSeverity[] IncidentSeverities => Enum.GetValues<IncidentSeverity>();
        public IncidentStatus[] IncidentStatuses => Enum.GetValues<IncidentStatus>();

        public IncidentDetailViewModel(IHealthSafetyService hseqService, IAuthService authService, ConnectionSettings settings)
        {
            _hseqService = hseqService;
            _authService = authService;
            _settings = settings;
            Title = "Incident Report";
        }

        public void Initialize(Incident incident, IEnumerable<IncidentPhotoDto>? photos = null, IEnumerable<IncidentDocumentDto>? docs = null)
        {
            Incident = incident;
            Photos = new ObservableCollection<IncidentPhotoDto>(photos ?? Enumerable.Empty<IncidentPhotoDto>());
            Documents = new ObservableCollection<IncidentDocumentDto>(docs ?? Enumerable.Empty<IncidentDocumentDto>());
        }

        [RelayCommand]
        public async Task SaveIncident()
        {
            if (string.IsNullOrWhiteSpace(Incident.Description) || string.IsNullOrWhiteSpace(Incident.Location))
            {
                NotifyError("Validation", "Description and Location are required.");
                return;
            }

            try
            {
                IsBusy = true;
                BusyText = "Saving incident...";
                
                if (_authService.CurrentUser != null)
                {
                    Incident.ReportedByUserId = _authService.CurrentUser.Id.ToString();
                }

                var result = await _hseqService.CreateIncidentAsync(Incident);
                if (result != null)
                {
                    NotifySuccess("Success", "Incident saved successfully.");
                    
                    var summary = new IncidentSummaryDto 
                    { 
                        Id = result.Id, 
                        Date = result.Date,
                        Location = result.Location,
                        Status = result.Status,
                        Severity = result.Severity,
                        Type = result.Type
                    };

                    Close(summary);
                }
                else
                {
                    NotifyError("Error", "Failed to save incident.");
                }
            }
            catch (Exception ex)
            {
                NotifyError("Error", "An error occurred while saving.");
                System.Diagnostics.Debug.WriteLine(ex);
            }
            finally
            {
                IsBusy = false;
            }
        }

        [RelayCommand]
        public async Task UploadPhotos()
        {
            var dialog = new Microsoft.Win32.OpenFileDialog
            {
                Multiselect = true,
                Title = "Select photos to upload",
                Filter = "Image Files|*.jpg;*.jpeg;*.png;*.gif;*.bmp"
            };

            if (dialog.ShowDialog() == true)
            {
                IsBusy = true;
                try
                {
                    if (Incident.Id == Guid.Empty)
                    {
                        BusyText = "Saving report first...";
                        var created = await _hseqService.CreateIncidentAsync(Incident);
                        if (created == null)
                        {
                            NotifyError("Error", "Could not save incident to allow photo uploads.");
                            return;
                        }
                        Incident.Id = created.Id;
                    }

                    int count = 0;
                    foreach (var fileName in dialog.FileNames)
                    {
                        var fileInfo = new FileInfo(fileName);
                        BusyText = $"Uploading {fileInfo.Name}...";
                        using var stream = fileInfo.OpenRead();
                        var metadata = new IncidentPhoto
                        {
                            IncidentId = Incident.Id,
                            FileName = fileInfo.Name
                        };

                        var result = await _hseqService.UploadIncidentPhotoAsync(metadata, stream, fileInfo.Name);
                        if (result != null)
                        {
                            count++;
                            Photos.Add(result);
                        }
                    }

                    if (count > 0) NotifySuccess("Success", $"Uploaded {count} photo(s).");
                }
                catch (Exception ex)
                {
                    NotifyError("Error", "Failed to upload photo(s).");
                    System.Diagnostics.Debug.WriteLine(ex);
                }
                finally
                {
                    IsBusy = false;
                }
            }
        }

        [RelayCommand]
        public async Task DeletePhoto(IncidentPhotoDto photo)
        {
            if (photo == null) return;

            try
            {
                var success = await _hseqService.DeleteIncidentPhotoAsync(photo.Id);
                if (success)
                {
                    Photos.Remove(photo);
                    NotifySuccess("Deleted", "Photo removed.");
                }
                else
                {
                    NotifyError("Error", "Failed to delete photo.");
                }
            }
            catch (Exception ex)
            {
                NotifyError("Error", "Exception deleting photo.");
                System.Diagnostics.Debug.WriteLine(ex);
            }
        }

        [RelayCommand]
        public void ViewAttachment(string? filePath)
        {
            if (string.IsNullOrEmpty(filePath)) return;

            try
            {
                var baseUrl = _settings.ApiBaseUrl.TrimEnd('/');
                var fullUrl = filePath.StartsWith("http") ? filePath : $"{baseUrl}/{filePath.TrimStart('/')}";

                System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo
                {
                    FileName = fullUrl,
                    UseShellExecute = true
                });
            }
            catch (Exception ex)
            {
                NotifyError("Error", "Could not open attachment.");
                System.Diagnostics.Debug.WriteLine($"ViewAttachment Error: {ex}");
            }
        }

        [RelayCommand]
        public async Task UploadDocuments()
        {
            var dialog = new Microsoft.Win32.OpenFileDialog
            {
                Multiselect = true,
                Title = "Select documents to upload"
            };

            if (dialog.ShowDialog() == true)
            {
                IsBusy = true;
                try
                {
                    if (Incident.Id == Guid.Empty)
                    {
                        BusyText = "Saving report first...";
                        var created = await _hseqService.CreateIncidentAsync(Incident);
                        if (created == null)
                        {
                            NotifyError("Error", "Could not save incident to allow document uploads.");
                            return;
                        }
                        Incident.Id = created.Id;
                    }

                    int count = 0;
                    foreach (var fileName in dialog.FileNames)
                    {
                        var fileInfo = new FileInfo(fileName);
                        BusyText = $"Uploading {fileInfo.Name}...";
                        using var stream = fileInfo.OpenRead();
                        var result = await _hseqService.UploadIncidentDocumentAsync(Incident.Id, stream, fileInfo.Name);
                        if (result != null)
                        {
                            count++;
                            Documents.Add(result);
                        }
                    }

                    if (count > 0) NotifySuccess("Success", $"Uploaded {count} document(s).");
                }
                catch (Exception ex)
                {
                    NotifyError("Error", "Failed to upload document(s).");
                    System.Diagnostics.Debug.WriteLine(ex);
                }
                finally
                {
                    IsBusy = false;
                }
            }
        }

        [RelayCommand]
        public async Task DeleteDocument(IncidentDocumentDto doc)
        {
            if (doc == null) return;

            try
            {
                var success = await _hseqService.DeleteIncidentDocumentAsync(doc.Id);
                if (success)
                {
                    Documents.Remove(doc);
                    NotifySuccess("Deleted", "Document removed.");
                }
                else
                {
                    NotifyError("Error", "Failed to delete document.");
                }
            }
            catch (Exception ex)
            {
                NotifyError("Error", "Exception deleting document.");
                System.Diagnostics.Debug.WriteLine(ex);
            }
        }
    }
}
