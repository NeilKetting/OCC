...
namespace OCC.Shared.Models
{
    ...
    /// <b>How:</b> Orders contain multiple <see cref="OrderLine"/> items. They can be for
    /// procurement (<see cref="OrderType.PurchaseOrder"/>) or project-specific allocation
    /// (<see cref="OrderType.PickingOrder"/>).
    /// Note: <c>Lines</c> is a plain <see cref="List{T}"/> intentionally — ObservableCollection
    /// belongs in the WPF UI layer (<c>OrderWrapper</c>), not in the shared domain model.
    /// </remarks>
    public class Order : BaseEntity
    {
        ...
        public string SupplierName { get; set; } = string.Empty; // Denormalized snapshot name

        // Picking Order
        public Guid? CustomerId { get; set; }

        // --- Snapshot Data (Address/VAT/Phone at time of order) ---
        // Generic to support both Supplier (PO) and Picking (PK) order types
        ...
        // --- Content ---
        /// <summary>
        /// The line items of this order. Uses <see cref="List{T}"/> — not ObservableCollection.
        /// Observable UI binding is handled by <c>OrderWrapper</c> in the WPF client.
        /// EF Core navigates this collection without any specific collection type requirement.
        /// </summary>
