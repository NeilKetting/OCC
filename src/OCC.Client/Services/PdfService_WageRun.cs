            ...
            container.Table(table =>
            {
                ...
                foreach (var line in lines)
                {
                    ...
                    if (line.IncentiveSupervisor > 0)
                    {
                        ...
                        table.Cell().Element(SubRowStyle).AlignRight().Text("SUPERVISOR FEE").Bold().FontSize(5.5f);
                        table.Cell().Element(SubRowStyle).AlignRight().Text($"R {line.IncentiveSupervisor:F2}").Bold().FontSize(5.5f);
